...
const ANIMATION_START_DELAY = 1950
const REMOVE_STAGE_DELAY = 700
const APPEAR_DELAY = 50
const COLOR_PULSE_INTERVAL = 1000
...
export const createBubble = (message, theme) => {
  ...
  const pulseAccent = () => {
    ...
    const current = bubble.style.borderColor

    const isTransparent = current === 'transparent'

    bubble.style.borderColor = isTransparent ? accent : 'transparent'
    bubble.style.boxShadow = isTransparent ? `0 4px 10px ${accent}66` : `0 4px 10px #00000066`

    bubble.style.color = isTransparent ? accent : theme.colors[colorThemeKeys.FOREGROUND]
  }

  const animateOutAndRemove = async () => {
    clearAllTimers()
    bubble.style.opacity = '0'
    bubble.style.pointerEvents = 'none'
    bubble.style.right = '-100%'

    await delay(REMOVE_STAGE_DELAY)
  ...
  animationTimeoutId = setTimeout(() => {
    ...
    animationIntervalId = setInterval(pulseAccent, COLOR_PULSE_INTERVAL)
