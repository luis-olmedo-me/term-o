...
export const createTableLogActionsCreatorPerRow = ({
  ...
  const isAboutToUnselectAll = activeColumnIds.length === 1

  return row => {
    const isActive = activeColumnIds.includes(row.id)
    ...
    return [
      ...
      },
      {
        ...
        Component: <Live active={isActive} />,
        disabled: isActive && isAboutToUnselectAll
      }
    ]
  }
}
